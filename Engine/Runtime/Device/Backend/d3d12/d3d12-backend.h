...
namespace std
{
template<> struct hash<redtea::device::TextureBindingKey>
{
	std::size_t operator()(redtea::device::TextureBindingKey const& s) const noexcept
	{
		return std::hash<redtea::device::Format>()(s.format)
			^ std::hash<redtea::device::TextureSubresourceSet>()(s)
			^ std::hash<bool>()(s.isReadOnlyDSV);
	}
};

template<> struct hash<redtea::device::BufferBindingKey>
{
	std::size_t operator()(redtea::device::BufferBindingKey const& s) const noexcept
	{
		return std::hash<redtea::device::Format>()(s.format)
			^ std::hash<redtea::device::BufferRange>()(s);
	}
};
}
...
namespace device {
    ...
    private:
        ...
        std::shared_ptr<BufferChunk> createChunk(size_t size) const;
    };

    class AccelStruct : public RefCounter<rt::IAccelStruct>
    {
    public:
        RefCountPtr<Buffer> dataBuffer;
    ...
    class CommandList final : public RefCounter<DXCommandList>
    {
    public:
