...
namespace device
{
    ...
    struct BufferRange
    {
        ...
        constexpr bool isEntireBuffer(const BufferDesc& desc) const { return (byteOffset == 0) && (byteSize == ~0ull || byteSize == desc.byteSize); }
    ...
    ENUM_CLASS_FLAG_OPERATORS(FastGeometryShaderFlags)
    ...
    struct BlendState
    {
        ...
        bool usesConstantColor(uint32_t numTargets) const;
...
#define BINDING_LAYOUT_ITEM_INITIALIZER(TYPE) /* NOLINT(cppcoreguidelines-macro-usage) */ \
        ...
        BINDING_LAYOUT_ITEM_INITIALIZER(Texture_SRV)
        BINDING_LAYOUT_ITEM_INITIALIZER(Texture_UAV)
        BINDING_LAYOUT_ITEM_INITIALIZER(TypedBuffer_SRV)
        BINDING_LAYOUT_ITEM_INITIALIZER(TypedBuffer_UAV)
        BINDING_LAYOUT_ITEM_INITIALIZER(StructuredBuffer_SRV)
        BINDING_LAYOUT_ITEM_INITIALIZER(StructuredBuffer_UAV)
        BINDING_LAYOUT_ITEM_INITIALIZER(RawBuffer_SRV)
        BINDING_LAYOUT_ITEM_INITIALIZER(RawBuffer_UAV)
        BINDING_LAYOUT_ITEM_INITIALIZER(ConstantBuffer)
        BINDING_LAYOUT_ITEM_INITIALIZER(VolatileConstantBuffer)
        BINDING_LAYOUT_ITEM_INITIALIZER(Sampler)
        BINDING_LAYOUT_ITEM_INITIALIZER(RayTracingAccelStruct)
...
#undef BINDING_LAYOUT_ITEM_INITIALIZER
    };
    ...
    // The default values match the offsets used by the shaderCompiler tool.
    ...
    {
    public:
        virtual const BindingLayoutDesc* getDesc() const = 0;           // returns nullptr for bindless layouts
        virtual const BindlessLayoutDesc* getBindlessDesc() const = 0;  // returns nullptr for regular layouts
    };
    ...
    public:
        ...
        // Sets the necessary resource states for all targets of the framebuffer.
         void setResourceStatesForFramebuffer(IFramebuffer* framebuffer, bool enableDepthWrite);
	...
	public:
		...
		virtual TextureHandle GetCurrentBackendBuffer() = 0;
...
namespace std
{
    template<typename T> struct hash<redtea::device::RefCountPtr<T>>
    {
        std::size_t operator()(redtea::device::RefCountPtr<T> const& s) const noexcept
        {
            std::hash<T*> hash;
            return hash(s.Get());
        }
    ...
    template<> struct hash<redtea::device::TextureSubresourceSet>
    {
        std::size_t operator()(redtea::device::TextureSubresourceSet const& s) const noexcept
        {
            size_t hash = 0;
            redtea::device::hash_combine(hash, s.baseMipLevel);
            redtea::device::hash_combine(hash, s.numMipLevels);
            redtea::device::hash_combine(hash, s.baseArraySlice);
            redtea::device::hash_combine(hash, s.numArraySlices);
            return hash;
        }
    };

    template<> struct hash<redtea::device::BufferRange>
    {
        std::size_t operator()(redtea::device::BufferRange const& s) const noexcept
        {
            size_t hash = 0;
            redtea::device::hash_combine(hash, s.byteOffset);
            redtea::device::hash_combine(hash, s.byteSize);
            return hash;
        }
    };

    template<> struct hash<redtea::device::BindingSetItem>
    {
        std::size_t operator()(redtea::device::BindingSetItem const& s) const noexcept
        {
            size_t value = 0;
            redtea::device::hash_combine(value, s.resourceHandle);
            redtea::device::hash_combine(value, s.slot);
            redtea::device::hash_combine(value, s.type);
            redtea::device::hash_combine(value, s.dimension);
            redtea::device::hash_combine(value, s.format);
            redtea::device::hash_combine(value, s.rawData[0]);
            redtea::device::hash_combine(value, s.rawData[1]);
            return value;
        }
    };

    template<> struct hash<redtea::device::BindingSetDesc>
    {
        std::size_t operator()(redtea::device::BindingSetDesc const& s) const noexcept
        {
            size_t value = 0;
    ...
    template<> struct hash<redtea::device::FramebufferInfo>
    {
        std::size_t operator()(redtea::device::FramebufferInfo const& s) const noexcept
        {
            size_t hash = 0;
            for (auto format : s.colorFormats)
                redtea::device::hash_combine(hash, format);
            redtea::device::hash_combine(hash, s.depthFormat);
            redtea::device::hash_combine(hash, s.width);
            redtea::device::hash_combine(hash, s.height);
            redtea::device::hash_combine(hash, s.sampleCount);
            redtea::device::hash_combine(hash, s.sampleQuality);
            return hash;
        }
    };

    template<> struct hash<redtea::device::BlendState::RenderTarget>
    {
        std::size_t operator()(redtea::device::BlendState::RenderTarget const& s) const noexcept
        {
            size_t hash = 0;
            redtea::device::hash_combine(hash, s.blendEnable);
            redtea::device::hash_combine(hash, s.srcBlend);
            redtea::device::hash_combine(hash, s.destBlend);
            redtea::device::hash_combine(hash, s.blendOp);
            redtea::device::hash_combine(hash, s.srcBlendAlpha);
            redtea::device::hash_combine(hash, s.destBlendAlpha);
            redtea::device::hash_combine(hash, s.blendOpAlpha);
            redtea::device::hash_combine(hash, s.colorWriteMask);
            return hash;
        }
    };

    template<> struct hash<redtea::device::BlendState>
    {
        std::size_t operator()(redtea::device::BlendState const& s) const noexcept
        {
            size_t hash = 0;
            redtea::device::hash_combine(hash, s.alphaToCoverageEnable);
            for (const auto& target : s.targets)
                redtea::device::hash_combine(hash, target);
            return hash;
        }
    };
}
