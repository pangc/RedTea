...
namespace math {
	...
	public:
		...
		friend inline
		constexpr QUATERNION<T>  operator*(const QUATERNION<T>& q, const QUATERNION<RT>& r)
		{
			// could be written as:
			//  return QUATERNION<T>(
		...
		template<typename RT>
		friend inline
			constexpr Vector3<T>  operator*(const QUATERNION<T>& q, const Vector3<RT>& v)
		{
		 ...
		 *
		 */
		friend inline constexpr QUATERNION<T>  operator*(QUATERNION<T> q, T scalar)
		{
			// don't pass q by reference because we need a copy anyways
			return q *= scalar;
		}

		friend inline constexpr QUATERNION<T> operator*(T scalar, QUATERNION<T> q)
		{
			// don't pass q by reference because we need a copy anyways
			return q *= scalar;
		}
	...
	public:
		...
		template<typename RT>
		friend inline constexpr T dot(const QUATERNION<T>& p, const QUATERNION<RT>& q)
		{
			return p.x * q.x +
				p.y * q.y +
				p.z * q.z +
		...
		friend inline T norm(const QUATERNION<T>& q)
		{
			return std::sqrt(dot(q, q));
		}

		friend inline T length(const QUATERNION<T>& q)
		{
			return norm(q);
		}

		friend inline constexpr T length2(const QUATERNION<T>& q)
		{
			return dot(q, q);
		}
		...
		friend inline constexpr QUATERNION<T> conj(const QUATERNION<T>& q)
		{
			return QUATERNION<T>(q.w, -q.x, -q.y, -q.z);
		}

		friend inline constexpr QUATERNION<T> inverse(const QUATERNION<T>& q)
		{
			return conj(q) * (1 / dot(q, q));
		}

		friend inline constexpr T real(const QUATERNION<T>& q)
		{
			return q.w;
		}

		friend inline
			constexpr Vector3<T> imaginary(const QUATERNION<T>& q) {
			return q.xyz;
		}

		friend inline constexpr QUATERNION<T> unreal(const QUATERNION<T>& q)
		{
			return QUATERNION<T>(q.xyz, 0);
		}

		friend inline constexpr QUATERNION<T> cross(const QUATERNION<T>& p, const QUATERNION<T>& q) {
			return unreal(p * q);
		}

		friend inline QUATERNION<T> exp(const QUATERNION<T>& q)
		{
			const T nq(norm(q.xyz));
			return std::exp(q.w) * QUATERNION<T>((sin(nq) / nq) * q.xyz, cos(nq));
		}

		friend inline QUATERNION<T> log(const QUATERNION<T>& q)
		{
			const T nq(norm(q));
			return QUATERNION<T>((std::acos(q.w / nq) / norm(q.xyz)) * q.xyz, std::log(nq));
		}

		friend inline QUATERNION<T> pow(const QUATERNION<T>& q, T a)
		{
		...
		friend inline QUATERNION<T> slerp(const QUATERNION<T>& p, const QUATERNION<T>& q, T t)
		{
		...
		friend inline constexpr QUATERNION<T> lerp(const QUATERNION<T>& p, const QUATERNION<T>& q, T t)
		{
			return ((1 - t) * p) + (t * q);
		}

		friend inline constexpr QUATERNION<T> nlerp(const QUATERNION<T>& p, const QUATERNION<T>& q, T t)
		{
			return normalize(lerp(p, q, t));
		}

		friend inline constexpr QUATERNION<T> positive(const QUATERNION<T>& q)
		{
			return q.w < 0 ? -q : q;
		}
	};
